...
-- ============================================
-- SCAN HEIGHT OFFSET SYSTEM (ABOVE GROUND)
-- ============================================
local SCAN_HEIGHT_OFFSET = 200 -- 200 studs ABOVE GROUND during scan (was -100)
...
-- ============================================
-- VOID PROTECTION - UPDATED FOR ABOVE GROUND SCANNING
-- ============================================
task.spawn(function()
	while true do
		task.wait(1)
		...
		-- When scanning, we're intentionally above ground
...
local function TeleportToMiniChestAbove(roomUID, roomIndex, actionType)
	...
	-- Use normal height offset (not above ground)
...
-- ============================================
-- SCAN FUNCTION - MAX 400 ROOMS - WITH 200 STUD HEIGHT OFFSET (ABOVE GROUND)
-- ============================================
local function Scan()
	if _G.IsScanning == true then
	...
	_G.IsScanning = true
	_G.IsScanningMode = true -- Enable scanning mode (above ground)

	local message = createMessage("Exploring the backrooms...")
	print("Starting FAST scan (200 studs ABOVE GROUND)...")
		...
		if room then
			...
			if not _G.Teleporting then
				...
				if character then
					local rootPart = character:FindFirstChild("HumanoidRootPart")
					if rootPart then
						-- Use hallway position if available, with 200 stud height offset (ABOVE GROUND)
						local teleportPos = room.HallwayPos or (room.Position + Vector3.new(0, 0, 0))
						-- Add height offset for scanning (ABOVE GROUND)
...
print("   • Teleports 200 studs ABOVE GROUND during scan")
